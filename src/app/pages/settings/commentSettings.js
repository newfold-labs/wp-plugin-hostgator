...
const OldPostsComments = ({ setError, notify }) => {
	...
	const toggleDisableCommentsOldPosts = () => {
		hostgatorSettingsApiFetch({ disableCommentsOldPosts: !disableCommentsOldPosts }, setError, (response) => {
...
const CloseCommentsDays = ({ setError, notify }) => {
	...
	const closeCommentsDaysNoticeTitle = () => {
		return (
			__('Comments setting saved ', 'wp-plugin-hostgator')
		);
	};
	...
	const closeCommentsDaysNoticeText = () => {
		...
		return (
			__('Comments on posts are disabled after ', 'wp-plugin-hostgator') +
			closeCommentsDays +
			_n(' day.', ' days.', parseInt(closeCommentsDays), 'wp-plugin-hostgator')
		);
	};
	...
	const handleCloseCommentsDaysChange = (value) => {
		hostgatorSettingsApiFetch({ closeCommentsDays: value }, setError, (response) => {
...
const CommentsPerPage = ({ setError, notify }) => {
	...
	const handleCommentsPerPageChange = (value) => {
		hostgatorSettingsApiFetch({ commentsPerPage: value }, setError, (response) => {
	...
	return (
		<SelectField
			...
			label={__('Display ', 'wp-plugin-hostgator') + commentsPerPage +
